 ...
 * @var yii\web\view;
 */
$this->registerCssFile('style/home.css',['depends'=>['frontend\assets\IndexAsset']]);
$this->registerCssFile('style/order.css',['depends'=>['frontend\assets\IndexAsset']]);
?>
	<!-- 头部 start -->
		...
		<div class="content fl ml10">
			...
			<div class="order_bd mt10">
				<table class="orders">
					...
					<tbody>
                        <?php foreach($order_goods as $detail):?>
						<tr>
							<td><a href=""><?=$detail->id?></a></td>
							<td><a href=""><?=\yii\helpers\Html::img('http://admin.yiishop.com'.$detail->logo)?></a></td>
							<td><?=$detail->order->name?></td>/
							<td><?=$detail->total?></td>
							<td><?=date('Y-m-d H:i:s',$detail->order->create_time)?></td>
							<td><?=$detail->order->status?'正常':'过期'?></td>
							<td><a href="<?=\yii\helpers\Url::to(['order/del','id'=>$detail->id])?>">删除</a></td>
						</tr>
                    <?php endforeach;?>
					</tbody>
				</table>
